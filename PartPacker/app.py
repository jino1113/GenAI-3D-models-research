...
def batch_generate(file_list, num_steps, cfg_scale, grid_res, randomize, seed, simplify, target_faces):
    seg_results = []
    model_results = []

    for file in file_list:
        seg_img = process_image(file.name)
        actual_seed = get_random_seed(randomize, seed)
        model_path = process_3d(seg_img, num_steps, cfg_scale, grid_res, actual_seed, simplify, target_faces)
        seg_results.append(seg_img)
        model_results.append(model_path)

    return seg_results, model_results
...
with block:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            with gr.Row():
                ...
                input_images = gr.File(label="Input Images", file_types=["image"], file_count="multiple")
                seg_gallery = gr.Gallery(label="Segmentation Results", show_label=True, columns=2, height=300)
                model_gallery = gr.Gallery(label="3D Model Outputs", show_label=True, columns=2, height=300)
    ...
    # Connect button to batch_generate
    button_gen.click(
        batch_generate,
        inputs=[
            input_images, num_steps, cfg_scale, input_grid_res,
            randomize_seed, seed, simplify_mesh, target_num_faces
        ],
        outputs=[seg_gallery, model_gallery]
    )

python gradio_app.py --enable_t23d --low_vram_mode# Hunyuan 3D is licensed under the TENCENT HUNYUAN NON-COMMERCIAL LICENSE AGREEMENT
...
def build_app():
    ...
    with gr.Blocks(theme=gr.themes.Base(), title='Hunyuan-3D-2.0', analytics_enabled=False, css=custom_css) as demo:
        ...
        def on_export_click(file_out, file_out2, file_type, reduce_face, export_texture, target_face_num):
            ...
            if export_texture:
                mesh = trimesh.load(file_out2, file_type=file_type)
                save_folder = gen_save_folder()
                ...
                mesh = trimesh.load(file_out, file_type=file_type)
                mesh = floater_remove_worker(mesh)
                mesh = degenerate_face_remove_worker(mesh)
                if reduce_face:
